...
## Setup and run
1. Make sure to have python ```3.12+``` installed and ensure its its ```PATH``` alias is ```python3```. Unless it's not ```python3```, adapt ```run.sh``` file to a relevant alias of python.
